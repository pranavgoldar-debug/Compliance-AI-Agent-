...
#
# Poll public regulator RSS / Atom feeds on a schedule, dedupe by GUID,
# and surface new items in-app so the team finds out about new filings
# without having to read every regulator's website by hand.
# ---------------------------------------------------------------------------
class RegulationFeed(Base):
    __tablename__ = "regulation_feeds"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    jurisdiction_code: Mapped[str] = mapped_column(String(8), nullable=False, index=True)
    url: Mapped[str] = mapped_column(String(1024), nullable=False)
    # "rss" | "atom" — autodetected on first poll, stored for the next.
    feed_type: Mapped[str] = mapped_column(String(16), nullable=False, default="rss")
    ...
    items: Mapped[list["RegulationFeedItem"]] = relationship(
        "RegulationFeedItem",
        back_populates="feed",
...
class RegulationFeedItem(Base):
    __tablename__ = "regulation_feed_items"
    __table_args__ = (
    ...
    feed_id: Mapped[int] = mapped_column(
        ForeignKey("regulation_feeds.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    # Stable per-feed identifier — falls back to the item link if the feed
    # doesn't carry an explicit GUID.
    ...
    # Set when an admin used this item to seed a new rule — links the
    # rule back to its discovery source.
    promoted_rule_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("rules.id", ondelete="SET NULL"), nullable=True
    )

    feed: Mapped[RegulationFeed] = relationship("RegulationFeed", back_populates="items")
