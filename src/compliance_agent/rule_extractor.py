...
- Emit FILINGS / returns / reports / registrations — not internal program duties. Ongoing housekeeping that is not itself submitted (maintaining policies, running transaction monitoring, retaining records, keeping a risk assessment current) should be folded into the relevant compliance-program obligation, not listed as many separate "Continuous" items. EXCEPTION: a DISCRETE, mandated obligation that has its OWN trigger or fixed cadence IS its own Rule even when nothing is submitted to a regulator — e.g. obtaining and renewing criminal-record / fit-and-proper / background checks for directors, senior officers and significant owners, or a periodic board attestation. Emit these as their own items; do NOT fold them away as "internal housekeeping".
...
- "Safeguarding Return (REP027)", "REP027 Safeguarding Return", "REP027 (Payment Services Directive safeguarding return)", "Safeguarding Return", "Monthly Safeguarding Return" — the MONTHLY safeguarding return (distinct from the annual audit above); keep the name "REP027 (Payment Services Directive safeguarding return)".
