...
_UK_FCA_RECALL = (
    ...
    "include the SPECIFIC RegData returns it owes, where applicable, naming each "
    "EXACTLY in the 'Name (code)' form shown for `name`:\n"
    "- Capital adequacy: Capital Adequacy Return (FSA056) — the Authorised "
    "Payment Institution capital-adequacy return — or the equivalent own-funds "
    "return for EMIs / investment firms.\n"
    "- Safeguarding (if it safeguards customer funds): the monthly "
    "REP027 (Payment Services Directive safeguarding return) AND the Annual "
    "Safeguarding Audit Report by an independent auditor. Use these exact "
    "filing names for `name`.\n"
    "- Financial resilience: Baseline Financial Resilience Report (FIN073) "
    "(frequency QUARTERLY — submitted each financial quarter, not annually).\n"
    "- Financial crime: Annual Financial Crime Report (REP-CRIM).\n"
    "- Ownership: Annual Controllers Report (REP002); Annual Close Links Report "
    "(REP001).\n"
    "- Conduct & operations: Payments Fraud Report (REP017, half-yearly); "
    "Operational and Security Risk Report (REP018); Complaints Return "
...
_US_RECALL = (
    ...
    "- State corporate / property filings — for EACH state of registration, "
    "list each as its OWN item, naming the state: the state annual report / "
    "registered-agent filing; any state franchise tax (e.g. Delaware LLC "
    "franchise tax); and — where the state levies one — the BUSINESS PERSONAL "
    "PROPERTY / tangible-property tax return as a SEPARATE item, even when the "
    "state files it on the SAME form as the annual report. Maryland is the "
    "classic case: the SDAT Annual Report and the Business Personal Property "
    "Return are filed together on Form 1, but the Business Personal Property "
    "Return is a DISTINCT obligation — list it separately, do NOT fold it into "
    "the generic annual report.\n"
