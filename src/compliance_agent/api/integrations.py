...
@cron_router.api_route("/send-reminders", methods=["GET", "POST"])
def trigger_send_reminders(token: str = _Query(..., description="Must equal CRON_TOKEN.")) -> dict:
    """Run the daily reminder engine (email + Slack escalation ladder,
    due-day pings, overdue chasers + MLRO/CFO escalation). Point a free
    external scheduler at this once a day — without it NO reminder ever
    fires on a host with no shell cron."""
    ...
    return {
        "ok": True,
        "sent": len(results),
        "emails": sum(1 for r in results if r.email_sent),
        "slack": sum(1 for r in results if r.slack_sent),
    }
